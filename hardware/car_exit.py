...
def handle_exit(plate_number):
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            if row['car_plate'] == plate_number and row['payment status'] == '1':
                # Update exit_time for the matching record
                row['exit_time'] = current_time
                updated = True
            rows.append(row)
    ...
    if updated:
        ...
        print(f"[ACCESS GRANTED] Exit recorded for {plate_number} at {current_time}")
        return True
    else:
